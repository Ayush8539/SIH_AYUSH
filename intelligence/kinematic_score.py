...
class KinematicThreatScore:
    ...
    def reasons(self) -> list[str]:
        ...
        if self.proximity >= 20.0:
            items.append(f"Close to Border: Distance {self.distance_px:.0f}px")
        if self.ttb_term >= 10.0 and not math.isinf(self.ttb_seconds):
            items.append(f"Imminent Breach: Time to border {self.ttb_seconds:.1f}s")
        if self.loiter_term >= 5.0:
            items.append(f"Loitering near boundary ({self.dwell_seconds:.0f}s)")
        if not items:
            if self.tier == "green":
                items.append("Normal activity (Routine movement)")
            else:
                items.append(f"Kinematic threat score ({self.total:.0f}/100)")
        return items
