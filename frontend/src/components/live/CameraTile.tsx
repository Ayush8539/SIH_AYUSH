      ...
      <div className="relative aspect-video w-full bg-[#05070a] flex items-center justify-center overflow-hidden">
        ...
        <div className="absolute top-3 left-3 z-20 flex flex-wrap items-center gap-2 max-w-[75%]">
          ...
          {tier && (
            <span
              className={cn(
                'font-mono text-[9px] px-1.5 py-0.5 rounded border font-bold uppercase tracking-wider',
                tier === 'red' && 'bg-accent-red/20 text-accent-red border-accent-red/50 shadow-[0_0_8px_rgba(239,68,68,0.3)]',
                tier === 'yellow' && 'bg-accent-yellow/20 text-accent-yellow border-accent-yellow/50 shadow-[0_0_8px_rgba(234,179,8,0.3)]',
                tier === 'green' && 'bg-accent-green/20 text-accent-green border-accent-green/50 shadow-[0_0_8px_rgba(34,197,94,0.3)]'
              )}
            >
              {tier === 'red' ? '🔴 RED' : tier === 'yellow' ? '🟡 YELLOW' : '🟢 GREEN'}
            </span>
          )}
          {activityGate && (
            <span
